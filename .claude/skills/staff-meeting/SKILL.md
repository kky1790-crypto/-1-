...
메인 Claude는 이 회의에서도 chief-of-staff로 남는다. 아래 전문 직원을 Agent 도구로 순서대로 호출하고, 최종 통합·보고는 메인 Claude가 직접 한다. 공통 원칙은 `.claude/rules/00-chief-of-staff.md`와 `docs/AI_EMPLOYEE_CHARTER.md`를 따른다. 직원별 세부 역할·입출력 규칙은 각 `.claude/agents/*.md`에 있으며, 이 문서에서 반복하지 않는다.
...
`.claude/rules/00-chief-of-staff.md` 3장(A/B/C 자동분류)에 따라 먼저 분류한다.
...
- **C(콘텐츠·문서 제작 업무)** 는 1~6단계에 이어 7~10단계(원고→근거검수→보완→제작 인계)까지 진행한다.

분류 결과와 이유는 최종 보고 끝에 한 줄로만 남긴다 (11-5장 형식).
...
12. **최종 통합 보고** — chief-of-staff가 직접 작성한다 (11장 6항목 형식).
...
**기본은 `.claude/rules/00-chief-of-staff.md` 11장의 6항목 짧은 보고를 쓴다** (결론/초안·결과물/중요 근거/남은 위험/결정할 것 최대 3개/승인 후 이어질 일). 직원별 긴 과정은 본문에 넣지 않는다.
...
보고 끝에는 11-5장 형식대로 업무분류/호출 기록/MEETING_STATUS를 한 줄씩만 남긴다. C유형이면 호출 기록에 원고·근거검수도 포함한다:
...
`docs/AI_EMPLOYEE_CHARTER.md` 3장 / `.claude/rules/00-chief-of-staff.md` 10-1장을 그대로 따른다. 검색 결과 제목·요약문만 확인한 내용은 SOURCE_EVIDENCE가 아니라 UNVERIFIED_LEAD로 표시한다. 법률·상표·자격 문제는 원문을 확인하지 못했으면 확정 판단하지 않는다.
...
`.claude/rules/00-chief-of-staff.md` 11-1~11-4장에 따라 GPT 검수용 보고서를 포함하고(실질적 업무이므로), 파일 변경 여부를 4가지로 구분해서 보고한다. 단순 확인·짧은 질의응답(A유형)에는 GPT 검수용 보고서와 회의 메타데이터를 만들지 않는다.
