...
## 2026-08-14 (6차 라운드) — 설치 없이 실행 (GitHub Codespaces 자동 설정)

- `.devcontainer/devcontainer.json` + `.devcontainer/start.sh` 신규 |
  Codespace 생성/재시작 시 review-admin·app 의존성을 자동 설치하고 두
  서버(4000/4100)를 백그라운드로 자동 실행, 포트가 열리면 브라우저 탭을
  자동으로 띄움(`onAutoForward: openBrowser`) | 사유: 원격 샌드박스의
  localhost는 사용자 브라우저에서 직접 열리지 않아, git/Node.js 설치 없이
  GitHub 계정만으로 클릭 몇 번으로 실행할 수 있는 경로가 필요했음 | 영향
  범위: 신규 설정 디렉터리 | 출처: 사용자(강윤) 지시
- 로컬에서 스크립트를 직접 실행해 정상 동작(설치·기동·재시작 시 기존
  프로세스 정리·데이터 무변경) 확인. Codespaces 자체(포트 포워딩·자동 탭
  열림 UI)는 이 세션에서 직접 재현할 수 없어 검증하지 못함 — 사용자가
  처음 열 때 확인 필요
- README.md 최상단에 "실행 방법(가장 쉬운 방법)" 섹션 추가: Codespaces
  클릭 경로를 1순위로, 로컬 설치를 대안으로 안내 | 영향 범위: README |
  출처: 사용자(강윤) 지시

## 2026-08-14 (5차 라운드) — 원자료 3건 등록 + 식구용 앱 프로토타입(app/)

- `inventory/items/`에 원자료 3건(INV-0001~0003) 등록, 전부
  `review_status: pending_review` | 3건 모두 이 프로젝트를 설계하는 대화 중
  사용자가 실제로 예로 든 문장만 사용(철학/원칙 성격 1, 운영/고객 관련 1,
  conflict 있는 항목 1) — 새 철학/규칙/운영 기준을 지어내지 않음. 화자/날짜
  불확실 → uncertainty/certainty를 낮게 표시 | 사유: review-admin 검수
  파이프라인을 사용자가 실제 데이터로 직접 써볼 수 있게 함 | 영향 범위:
  `inventory/items/` | 출처: 사용자(강윤) 지시,
  `review-admin/scripts/seed-initial-three.js`에 근거 기록
- `app/` 신규: Phase 3 식구용 앱 프로토타입 v1. 자체 데이터 계층 없이
  `review-admin/routes/publicApi.js`를 그대로 재사용(승인 필터·공개 필드
  화이트리스트가 코드베이스에 단 하나만 존재하도록) | 홈(검색+scope
  필터+최근 승인 목록), 상세(content_type→규칙/원칙/판단/철학/절차/지표/
  사례/의견/질문 배지+설명, 출처·권위 배지, 출처 불확실 시 경고 문구) |
  30~100개로 데이터가 늘어나도 그대로 쓸 수 있는 구조(검색+필터+카드 그리드) |
  테스트 3개(node:test, 승인 필터 재확인 포함) 통과 | 사유: 승인한 내용이
  실제로 어떻게 보이는지 직접 확인 가능하게 함 | 영향 범위: 신규 디렉터리 |
  출처: 사용자(강윤) 지시
- 검증: 3건 전부 pending_review 상태에서 `app/`이 빈 목록(친절한 안내
  문구)을 보여주는지 확인 → 1건을 review-admin에서 임시 승인해
  `app/`에서 실제 렌더링(배지/설명/출처 경고 포함)을 스크린샷으로 확인 →
  다시 pending_review로 되돌리고, 검증 목적으로 생긴 이력(`강윤`으로
  잘못 귀속될 수 있는 승인/취소 로그)은 삭제해 실제 사용자 행동만 이력에
  남도록 정리 | 영향 범위: 검증 절차 | 출처: 사용자(강윤) 지시
- docs/PRODUCT.md(Phase 1.5 신설, Phase 1/3 상태 갱신), docs/INVENTORY_MODEL.md,
  inventory/README.md, README.md를 새 상태와 동기화 | 영향 범위: 문서 전반 |
  출처: 사용자(강윤) 지시

다음 단계 (보류): 사용자가 이 3건짜리 구조(필드/버튼/문구/화면 구성)를
직접 써보고 "괜찮다"고 확인하기 전까지 나머지 원자료·콘텐츠를 추가하지
않는다.
